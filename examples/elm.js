...
Elm.Widget.Accordion = Elm.Widget.Accordion || {};
Elm.Widget.Accordion.make = function (_elm) {
   "use strict";
   _elm.Widget = _elm.Widget || {};
   _elm.Widget.Accordion = _elm.Widget.Accordion || {};
   if (_elm.Widget.Accordion.values) return _elm.Widget.Accordion.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var role = $Html$Attributes.attribute("role");
   _op["=>"] = F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};});
   var view = F2(function (accordion,entries) {
      var viewEntry = function (entry) {
         var entryPanel = A2($Html.div,_U.list([$Html$Attributes.$class("accordion-entry-panel"),role("tabpanel")]),_U.list([accordion.viewPanel(entry)]));
         var expanded = accordion.getExpanded(entry);
         var entryClass = $Html$Attributes.classList(_U.list([A2(_op["=>"],"accordion-entry",true)
                                                             ,A2(_op["=>"],"accordion-entry-state-expanded",expanded)
                                                             ,A2(_op["=>"],"accordion-entry-state-collapsed",$Basics.not(expanded))]));
         var entryHeader = A2($Html.div,
         _U.list([$Html$Attributes.$class("accordion-entry-header")
                 ,A3($Html$Events.on,
                 "click",
                 $Json$Decode.succeed({ctor: "_Tuple0"}),
                 function (_p0) {
                    return A2(accordion.setExpanded,$Basics.not(expanded),entry);
                 })]),
         _U.list([accordion.viewHeader(entry)]));
         return A2($Html.div,_U.list([entryClass,role("tab")]),_U.list([entryHeader,entryPanel]));
      };
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("accordion"),role("tablist"),A2($Html$Attributes.attribute,"aria-live","polite")]),
      A2($List.map,viewEntry,entries));
   });
   var Accordion = F4(function (a,b,c,d) {    return {viewHeader: a,viewPanel: b,setExpanded: c,getExpanded: d};});
   return _elm.Widget.Accordion.values = {_op: _op,view: view,Accordion: Accordion};
};
...
Elm.Main.make = function (_elm) {
   ...
   $Widget$Accordion = Elm.Widget.Accordion.make(_elm);
   var _op = {};
   var expandIf = F2(function (predicate,entry) {    return _U.update(entry,{expanded: predicate(entry)});});
   var update = F2(function (action,entries) {
      var _p0 = action;
      if (_p0.ctor === "NoOp") {
            return entries;
         } else {
            return A2($List.map,expandIf(function (_p1) {    var _p2 = _p1;return _U.eq(_p2.id,_p0._0.id);}),entries);
         }
   });
   var Expand = function (a) {    return {ctor: "Expand",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var view = F2(function (address,entries) {
      var accordionOpts = {viewHeader: function (_p3) {
                             return $Html.text(function (_) {    return _.heading;}(_p3));
                          }
                          ,viewPanel: function (_p4) {
                             return $Html.text(function (_) {    return _.body;}(_p4));
                          }
                          ,setExpanded: F2(function (expanded,entry) {    return A2($Signal.message,address,Expand(entry));})
                          ,getExpanded: function (_) {
                             return _.expanded;
                          }};
      return A2($Widget$Accordion.view,accordionOpts,entries);
   });
   var entries = _U.list([{id: 0,heading: "Top Thing",body: "Top Content",expanded: false}
                         ,{id: 1,heading: "Middle Thing",body: "Middle Content",expanded: true}
                         ,{id: 2,heading: "Bottom Thing",body: "Bottom Content",expanded: false}]);
   var main = $StartApp$Simple.start({model: entries,view: view,update: update});
   var AccordionEntry = F4(function (a,b,c,d) {    return {id: a,expanded: b,heading: c,body: d};});
   return _elm.Main.values = {_op: _op
                             ,AccordionEntry: AccordionEntry
                             ,main: main
                             ,entries: entries
                             ,view: view
                             ,NoOp: NoOp
